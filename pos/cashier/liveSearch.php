...
if (isset($_POST['input'])) {
	$i = $_POST['input'];
	$ful_total = 0;
	$receipt = Database::search("SELECT * FROM `rpos_payments` WHERE  `order_code` LIKE '{$i}%'");
...
?>
	...
	<tr style="font-weight: bold; background-color: #DDDDDD;">
		...
		<?php 
                            $f = Database::search("SELECT sum(pay_amt) AS total FROM `rpos_payments` WHERE `order_code` LIKE '{$i}%'");
